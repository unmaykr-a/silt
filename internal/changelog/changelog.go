...
var Releases = []Release{
	{
		Version: "1.2.0",
		Date:    "2026-09-15",
		Summary: "Authentication is editable from the settings screen, with a key that encrypts what is stored and a way back in when a save goes wrong.",
		Entries: []Entry{
			{Added, "All twenty authentication settings are editable under Settings → Authentication: the built-in account, forward auth and its trusted proxies, the whole OpenID Connect block, the session lifetimes and the cookie policy. Saving rebuilds the gate — the account is re-read, the proxy rules are rebuilt and provider discovery runs again — and sessions survive it, because they are rows in the database rather than signed cookies."},
			{Changed, "That was withheld before now on the argument that a UI able to edit the boundary in front of it would be a way in rather than a setting. What changed the answer is that only an administrator reaches that screen, and an administrator already holds the stronger controls: the whole-database backup, the password, every session. Withholding it bought nothing and cost a container recreate to fix a mistyped issuer."},
			{Added, "SILT_SETTINGS_RESET drops every setting saved from the UI at startup, so the install runs exactly what its environment says. This is the way back in from a save that locked you out, which is why it is read before the stored settings are and works without signing in."},
			...
			{Added, "A byte-scan test for those credentials, on the pattern the redaction sentinel already uses: plant a secret-shaped value in every secret-tagged setting, then scan the database file, its write-ahead log and a backup taken through the API. Querying would only prove the value is not in the column it was meant to be in; scanning proves it is not in the file that leaves the host."},
			{Fixed, "SILT_PASSWORD_HASH stays environment-only, and the reason is now stated where someone will read it: the variable exists to take the password out of the UI's hands for an install managed from a compose file, so a UI that could set it would defeat its only purpose."},
			{Changed, "Six of forty-five settings are environment-only now, and the package doc names each one and why — checked by a test, which is how the list came to be right after being written down wrong the first time."},
		},
	},
	{
		Version: "1.1.0",
		Date:    "2026-09-15",
		Summary: "Nineteen of forty-three settings are editable while Silt runs, and adding the twentieth is one struct tag.",
		Entries: []Entry{
			{Changed, "Which settings are editable is decided by a tag on the config field rather than by six hand-maintained lists. Adding one used to mean a field on an Overrides struct, an entry in a Fields slice, and a case in each of apply, Set, merge and clearFields — five of the six pure switch-case, with nothing comparing them to each other. Forgetting the merge case gave a setting that saved and then reverted on the next save of anything else, silently. Net 200 lines out of internal/settings."},
			{Added, "Five settings became editable, and none of them was protecting anything: the host name, the Docker endpoint, the compose-file size cap, the ingest rate limit and whether /metrics answers without a session. What kept them in the environment was the cost of adding a setting, written up afterwards as policy. Fourteen editable settings became nineteen."},
			{Fixed, "Renaming the host moves its history. SILT_HOST_NAME is the key the host row is keyed on, so changing it and restarting used to insert a second host and re-create every project under it — leaving all the history under the old label and none under the new one. Renaming back to a label used before finds a row already holding it, and two histories are left as two rather than merged on a guess."},
			{Added, "Changing the Docker endpoint redials rather than waiting for a restart. Closing the old transport is what ends the event stream, so the watcher's own reconnect loop picks up the new engine; an endpoint that will not dial is refused and the old one stays."},
			{Fixed, "A misspelt setting name is refused instead of ignored. The patch decoded into a struct and encoding/json drops what it does not recognise, so a PUT naming retention_dayz answered 200 and changed nothing."},
			{Security, "The settings export strips secrets by asking which fields are secret, rather than naming the two it knew about. The next secret added would have ridden out in a file people keep."},
			{Added, "Guards that hold for every setting rather than one at a time: each must round-trip through the HTTP API, be readable back, reset to the environment, appear in both halves of the OpenAPI schema and in nothing else, and carry a **restart** marker in the manual exactly when it is not editable — while every secret must be absent from the response. They are driven by the registry, so a setting added later is covered the moment it is tagged, and the build fails when a new setting has no test value. Each was verified by breaking what it guards."},
			{Changed, "Settings values are trimmed and de-blanked on the way in, so a list typed as \"TZ, \" stores one key rather than two, one of them empty."},
		},
	},
	{
		Version: "1.0.1",
		Date:    "2026-09-15",
		Summary: "A wiki that is the manual, a README that is an introduction, and tests that keep both from rotting.",
		Entries: []Entry{
			{Added, "Eighteen wiki pages, published to the GitHub wiki by CI from docs/wiki on every push to main. Install, configuration, authentication, redaction, compose capture, retention, notifications, ingest, backups, metrics, the screens, the architecture, the API, troubleshooting, FAQ, upgrading and development. The repository is the source of truth and the wiki is a render of it, for the same reason CHANGELOG.md is generated: two copies of the same prose disagree by the second edit."},
			{Changed, "The README is an introduction again — 461 lines to 191. What it is, what it does, a compose file, and where the rest lives. Everything it used to explain in collapsed sections now has a page with room to explain it properly."},
			{Added, "Tests for the wiki's shape, because a wiki rots in two ways that are invisible to whoever wrote it: a page stops being linked and nobody finds it again, and a link points at a page that was renamed — which GitHub renders as a working-looking link that 404s. Every page must be in the sidebar, reachable from Home, longer than a stub, and every internal link must resolve. Verified by breaking each of those four and watching the right test fail."},
			{Added, "The settings reference now exists in three places, so all three are checked against the struct the process actually reads. That guard is why the whole OpenID Connect block was found documented nowhere in 0.17.0."},
			{Added, "Workflows are parsed by the test suite. GitHub was the only other thing that parses them, and it does so after a push — so an invalid one is a red X on a commit that is already public. This exists because the wiki workflow was written with a multi-line commit message inside a `run: |` block, and an unindented continuation line ends a YAML block scalar: a parse error that looked entirely reasonable."},
		},
	},
	{
		Version: "1.0.0",
		Date:    "2026-09-15",
		Summary: "Every item in the v1 scope now exists, including the one that had been written down four ways and built none of them.",
		Entries: []Entry{
			{Added, "Silt watches your compose files. An edit is noticed within a second instead of on the next Docker event or the next reconcile — up to five minutes on the default cadence, and never at all on a quiet host, which is exactly the host where an un-applied edit is easiest to forget. The watch is on the directory rather than the file, because an atomic-save editor replaces the inode and a file-level watch goes deaf after the first save while continuing to report success. Writes are debounced, because one save is a truncate, several writes and a rename."},
			...
			{Fixed, "A failing assertion inside the collector test harness hung the whole package for the ten-minute test timeout instead of reporting a failure, because t.Fatal ends its goroutine without returning and the cleanup that stopped the collector never ran."},
			{Changed, "1.0.0 because the v1 scope in PROJECT.md Section 2 is complete, not because the version needed rounding up. The last item to land was the compose file watch — trigger 2 of the four in Section 5, which had been specified in the brief, listed in the locked tech stack, named in the repo layout, given a gotcha of its own, and left unbuilt for nineteen releases. It is the fourth thing in this project to be documented into existence without being written; the pattern is recorded as design note 111, along with the one kind of test that would have caught all four."},
		},
	},
	{
		Version: "0.19.0",
